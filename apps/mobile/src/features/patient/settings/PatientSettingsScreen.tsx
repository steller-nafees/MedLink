...
import { ChevronLeft, ChevronRight, Globe, HelpCircle, LogOut, ShieldCheck, Wifi, type LucideIcon } from "lucide-react-native";
import { theme } from "../../../theme";
import { clearAuthSession } from "../../../services/auth";
import { getNetworkOverride, setNetworkOverride, type NetworkOverride } from "../../sos/utils/offline-sync";
import { connectivityOptions, emergencyConnectivityCopy, languageOptions, settingsCopy, settingsVersion, type SettingsLanguage } from "./settingsMockData";
...
export default function PatientSettingsScreen() {
  ...
  useEffect(() => { void Promise.all([AsyncStorage.getItem(LANGUAGE_KEY), getNetworkOverride()]).then(([storedLanguage, storedNetwork]) => { if (storedLanguage === "en" || storedLanguage === "bn") setLanguage(storedLanguage); setNetwork(storedNetwork); }); }, []);
  ...
  return <View style={styles.screen}><StatusBar style="dark" /><ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 24 }]} showsVerticalScrollIndicator={false}>
