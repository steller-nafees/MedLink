...
const ambulanceListSchema = z.object({
    limit: z.coerce.number().int().min(1, "Limit must be at least 1")
        .max(100, "Limit cannot exceed 100").optional().default(20),
    offset: z.coerce.number().int().min(0, "Offset cannot be negative")
        .optional().default(0),
...
const nearbyAmbulanceSchema = z.object({
    radius: z.coerce.number().positive("Radius must be greater than 0")
        .max(100, "Radius cannot exceed 100 km").optional().default(10),
