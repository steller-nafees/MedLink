                ...
                6371 * acos(
                    LEAST(
                        ...
                        GREATEST(
                            ...
                            *
                            cos(
                                radians(h.longitude::double precision)
                                - radians($2)
                            )
                            +
                            sin(radians($1))
...
const getHospitalById = async (hospitalId) => {
    ...
    const icuWards = wardsResult.rows.filter(
        (ward) =>
    ...
    // --------------------------------------------------------
    // Calculate ICU information
    // --------------------------------------------------------

    const icuInformation = {
        totalBeds: icuWards.reduce(
            (total, ward) =>
                total + Number(ward.total_beds),
            0
        ),

        availableBeds: icuWards.reduce(
            (total, ward) =>
                total + Number(ward.available_beds),
            0
        ),

        occupiedBeds: icuWards.reduce(
            (total, ward) =>
                total + Number(ward.occupied_beds),
            0
        ),

        reservedBeds: icuWards.reduce(
            (total, ward) =>
                total + Number(ward.reserved_beds),
            0
        ),

        maintenanceBeds: icuWards.reduce(
            (total, ward) =>
                total + Number(ward.maintenance_beds),
            0
        ),
    ...
    // --------------------------------------------------------
    // Return complete hospital details
    // --------------------------------------------------------
