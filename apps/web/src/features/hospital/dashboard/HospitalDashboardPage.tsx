...
function RechartsLine(_props: Record<string, unknown>) {
	return <>
		<RechartsLegend wrapperStyle={{ fontSize: 11 }} />
		<RechartsLineBase type="monotone" dataKey="incoming" name="Incoming" stroke="#16a89c" strokeWidth={2.5} dot={{ r: 4, strokeWidth: 2, fill: "white" }} />
		<RechartsLineBase type="monotone" dataKey="completed" name="Completed" stroke="#2f80ed" strokeWidth={2.5} dot={{ r: 4, strokeWidth: 2, fill: "white" }} />
		<RechartsLineBase type="monotone" dataKey="rejected" name="Rejected" stroke="#e05252" strokeWidth={2.5} dot={{ r: 4, strokeWidth: 2, fill: "white" }} />
	</>;
}
...
export function HospitalDashboardPage() {
	...
	const icuAvailable = summary?.available_icu_beds ?? 0;
	...
	const weekly = (analytics?.weekly ?? []).map((item) => ({
		...item,
		incoming: Number(item.incoming) || 0,
		completed: Number(item.completed) || 0,
		rejected: Number(item.rejected) || 0,
	}));
	const bySeverity = (analytics?.bySeverity ?? []).map((item) => ({
		...item,
		value: Number(item.value) || 0,
		color: severityColors[item.name] ?? "var(--hospital-muted)",
	}));
	const totalWeeklyCases = weekly.reduce((total, item) => total + item.incoming, 0);
