...
import { approveEmergencyCase, approveReservation, getHospitalIncomingRequests } from "@/services/hospital.service";
...
export function HospitalRequestsPage() {
  ...
  async function approveRequest(request: HospitalServiceRequest) {
    ...
    setError(null);
    try {
      if (request.kind === "emergency") await approveEmergencyCase(request.id);
      else await approveReservation(request.id);
